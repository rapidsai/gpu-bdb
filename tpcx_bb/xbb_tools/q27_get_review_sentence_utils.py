## This file contains the review sentence gathering part of workflow
...
def get_review_sentence(tokenized_d,predicted_label_t,vocab2id,id2vocab,org_labels = [2,5]):
    """
        Given a tokenized_d and predicted_label_t
        we gather the  sentences that contain labels for either 5 or 2
    """
    ...
    output_mat = cp.zeros(shape=(len(org_senten_df['org_seq_row']),1024*2),dtype=np.int32)
...
def get_org_sentences(sentence_boundary_df,org_df):
    """
        Returns the sentences that contain org,
...
def get_output_sen_word_kernel(start_r_ar,start_c_ar,
    ...
    """
        Returns the output sentences given these
        TODO
    """
...
def get_org_df(pr_label_f,metadata_df,seq_len):
    ...
    flag =  (org_df['org_seq_col']>=org_df['start_index']) & (org_df['org_seq_col']<=org_df['stop_index'])
    org_df = org_df[flag]
